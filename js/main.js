// ============================================================
// GONZO SHOP — main.js (با آپلود عکس و محدودیت ۱۰۰ مگابایت)
// ============================================================
...
// CONFIG
// ============================================================
const API_BASE_URL = 'https://gonzo-admin.kabiri1489.workers.dev';
...
async function loadAccounts() {
    try {
        ...
        if (response.ok) {
            ...
            if (result.success && result.data && result.data.length > 0) {
                localStorage.setItem('gonzoShopAccounts', JSON.stringify(result.data));
    ...
    try {
        ...
        if (res.ok) {
            ...
            localStorage.setItem('gonzoShopAccounts', JSON.stringify(data));
            console.log('📂 داده‌ها از فایل محلی بارگذاری شد.');
            return data;
        }
    } catch (err) {
        console.warn('⚠️ فایل محلی در دسترس نیست:', err.message);
    }

    const cached = localStorage.getItem('gonzoShopAccounts');
    if (cached) {
        try {
    ...
    localStorage.setItem('gonzoShopAccounts', JSON.stringify(fallback));
    console.log('💾 داده‌های پیش‌فرض استفاده شد.');
    return fallback;
}
...
adminCodeSubmit.addEventListener('click', async function() {
    ...
    try {
        ...
        if (result.success && result.token) {
            ...
            adminLoginError.textContent = result.error || 'کد مدیریت اشتباه است.';
            adminLoginError.classList.add('show');
            adminCodeInput.value = '';
            adminCodeInput.focus();
        }
    } catch (err) {
        adminLoginError.textContent = 'خطا در ارتباط با سرور.';
        adminLoginError.classList.add('show');
...
function loadAdminConfig() {
    adminCardNumber.value = CONFIG.cardNumber || '5022 2915 2835 2154';
    adminSoroushId.value = CONFIG.adminSoroushId || '@ID_KYAN';
}
adminSaveCard.addEventListener('click', function() {
    ...
    localStorage.setItem('gonzoShopConfig', JSON.stringify(config));
...
// ============================================================
// تابع آپلود عکس با بررسی حجم و پیام هوشمند
// ============================================================
async function uploadImage(file) {
    ...
    // بررسی حجم (حداکثر ۱۰۰ مگابایت)
    if (file.size > MAX_IMAGE_SIZE) {
        const sizeInMB = (file.size / (1024 * 1024)).toFixed(1);
        const errorMsg = `❌ حجم عکس (${sizeInMB} مگابایت) بیشتر از حد مجاز (۱۰۰ مگابایت) است.\n\n` +
                         `🔧 برای کاهش حجم عکس، از ابزارهای زیر استفاده کنید:\n` +
                         `• TinyPNG (https://tinypng.com/)\n` +
                         `• Squoosh (https://squoosh.app/)\n` +
                         `• Photoshop یا GIMP (Export as WebP یا JPEG با کیفیت پایین)\n` +
                         `• ابزارهای آنلاین دیگر (مثل ILoveIMG, Compressor.io)\n\n` +
                         `پس از کاهش حجم، دوباره آپلود کنید.`;
        alert(errorMsg);
        return null;
    }
    ...
    try {
        ...
        // بررسی خطای حجم از سرور (در صورت ارسال)
        if (err.message.includes('exceeds') || err.message.includes('limit')) {
            ...
            alert(`❌ حجم عکس (${sizeInMB} مگابایت) بیشتر از حد مجاز (۱۰۰ مگابایت) است.\n\n` +
                  `🔧 لطفاً عکس را با ابزارهای زیر کوچک‌تر کنید:\n` +
                  `• TinyPNG (https://tinypng.com/)\n` +
                  `• Squoosh (https://squoosh.app/)\n` +
                  `• Photoshop یا GIMP\n\n` +
...
async function saveAllToServer() {
    ...
    try {
        ...
        localStorage.setItem('gonzoShopAccounts', JSON.stringify(accounts));
...
function deleteAccount(id) {
    ...
    localStorage.setItem('gonzoShopAccounts', JSON.stringify(allAccounts));
    renderAccounts(allAccounts);
    renderAdminAccounts();
...
// ============================================================
// ADMIN ADD/EDIT FORM (با آپلود عکس و پیام هوشمند)
// ============================================================
...
console.log('✅ GONZO SHOP loaded. اتصال به GitHub از طریق Worker با JWT و آپلود عکس فعال است.');