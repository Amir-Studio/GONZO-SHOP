// ============================================================
// GONZO SHOP — main.js (نسخه نهایی با اتصال به GitHub)
// ============================================================
...
async function loadAccounts() {
    try {
        // 1. ابتدا از Worker بخوان
        const response = await fetch(`${API_BASE_URL}/api/accounts`);
        if (response.ok) {
            const result = await response.json();
            if (result.success && result.data && result.data.length > 0) {
                const filtered = result.data.filter(a => a.id !== 'AK-006');
                localStorage.setItem('gonzoShopAccounts', JSON.stringify(filtered));
    ...
    // 2. اگر Worker کار نکرد، از فایل محلی بخوان
    try {
        const res = await fetch('data/accounts.json');
        if (res.ok) {
            const data = await res.json();
            const filtered = data.filter(a => a.id !== 'AK-006');
            localStorage.setItem('gonzoShopAccounts', JSON.stringify(filtered));
    ...
    // 3. اگر همه چیز fail شد، از localStorage بخوان
...
async function saveAllToServer() {
    ...
    try {
        ...
        setTimeout(() => {
            btn.innerHTML = '💾 ذخیره همه تغییرات روی سرور (GitHub)';
            btn.disabled = false;
        }, 3000);
    } catch (err) {
